...
#'@description
#'Calculates the height of buildings on two sides of streets with two options available.
...
#'@usage
#'Building_height(building, street, option = ("fixed", "street"), k = )
#'
...
#'@param option Defines the method by which building height is calculated.
...
#'@details
#'Calculate building height with two options available.
...
#'street<-data(street)
#'building_height (building, street, option = "fixed", k = 40)

#'building <-data(building)
#'street<-data(street)
#'building_height (building, street, option = "street", k = 40)
#'## End(Not run)
building_height <- function(city,road,option,k){
  ...
  height_mean <- character (nrow(road))
  height_weighted.mean <- character (nrow(road))
  height_median <- character (nrow(road))
  height_min <- character (nrow(road))
  height_max <- character (nrow(road))
  height_sd <- character (nrow(road))
  ...
  if (option == "street"){
    ...
    for (i in (1:length(road))){
      ...
      if (nrow(intersect) == 0) {
        ...
        #计算指标
        height_mean[i] <- mean(intersect$floors)
        height_weighted.mean[i] <- weighted.mean(intersect$floors,intersect$length)
        height_median[i] <- median(intersect$floors)
        height_min[i] <- min(intersect$floors)
        height_max[i] <- max(intersect$floors)
        height_sd[i] <- sd(intersect$floors)
      }
    }
  }
  ...
  road$height_mean <- height_mean
  road$height_mean <- height_weighted.mean
  road$height_median <- height_median
  road$height_min <- height_min
  road$height_max <- height_max
  road$height_sd <- height_sd
}
